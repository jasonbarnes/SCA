...
def usage():
    ...
    """
def print_triplets(trace):
    #if len(trace.getPackets()) == 0:
    #    return
    print ":"+str(trace.getId())
    for pkt in trace.getPackets():
        print str(pkt.getDirection())+","+str(pkt.getTime())+","+str(pkt.getLength())+",0"
    return
        ...
        for webpageId in webpageIds:
            ...
            for w in [webpageTrain, webpageTest]:
                for trace in w.getTraces():
                    ...
                    if instance:
		    	print_triplets(traceWithCountermeasure)
                        if i==0:
