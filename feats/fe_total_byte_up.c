...
extern int total_byte_up_fe_num(struct fe_config_list *fe_list){
	...
	set = cs_load_set("size_total_up.dat");
	if(set == NULL){
		return 0;
	}
...
extern int total_byte_up_fe_extract(struct fe_config_list *fe_list, double *feature_vector, int feature_vector_start_index, struct t_trace *trace){
	double *list;
	int list_max;
	int i;
	struct countable_set *set;
	list = feature_vector + feature_vector_start_index;
	list_max = total_byte_up_fe_num(fe_list);
	set = cs_load_set("size_total_up.dat");
	if(set == NULL){
		return EXIT_FAILURE;
	}
