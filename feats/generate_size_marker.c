...
feature_extraction.c and fe_size_markers.c to work correctly:
The number of features expected by fe_size_markers.c
...
void usage(){
	printf("usage:\ngenerate_html_marker <pcap_file> <size_marker_up.dat> <size_marker_down.dat>\n");
}
...
void generate_set(const struct pcap_pkthdr *pkthdr, const u_char *packet, struct iphdr *ip, struct tcphdr *tcp, void *data){
	...
	if(((dat->src == ip->saddr) && (dat->directionCursor == 1)) || ((dat->src != ip->saddr) && (dat->directionCursor == 0))){
		if(dat->directionCursor == 0){
			...
			cs_add_key(dat->down_set, (int)round_double((double)dat->dataCursor, 600.0));
		}
		if(dat->src == ip->saddr){
			dat->directionCursor = 0;
		}
...
int main(int argc, char **argv){
	...
	int i;
	dat = (struct size_marker_dat *)malloc(sizeof(struct size_marker_dat));
	dat->num_packets = 0;
	dat->done = 0;
	dat->src = 0;
	dat->dst = 0;
