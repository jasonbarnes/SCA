...
void usage(){
	printf("usage:\ngenerate_total_marker <trip_file> <size_total_up.dat> <size_total_down.dat>\n");
}
...
int generate_set_trace(struct t_trace *trace, void *data){
	...
	cs_add_key(info->size_up_set, (int)round_double(info->total_up, 10000.0));
	cs_add_key(info->size_down_set, (int)round_double(info->total_down, 10000.0));
	return EXIT_SUCCESS;
}
